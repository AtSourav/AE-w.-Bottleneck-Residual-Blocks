...
This is identical to the Residual_blocks module only with the build method 
implemented as we want to use it to subclass the Model and Layer classes.
"""
...
class conv2d_block(layers.Layer):     # it's not the most general conv2d layer we could use.
	...
	def build(self, input_shape):
		...
		self.conv2d = layers.Conv2D(filters=self.num_filters, 
...
class conv2dtrans_block(layers.Layer):    
	...
	def build(self, input_shape):
		...
		self.conv2dtrans = layers.Conv2DTranspose(filters=self.num_filters, 
											kernel_size=self.kernel, 
											strides=self.strides, 
								            padding=self.padding, 
											kernel_initializer=self.kernel_initializer, 
...
class residual_conv2D_block(layers.Layer):
	...
	def build(self, input_shape):
		...
		self.conv_bn = conv2d_block(self.num_filters, 
							  kernel=self.kernel, 
							  kernel_initializer=self.kernel_initializer, 
							  strides=self.strides, 
							  padding=self.padding, 
		...
		self.conv_wobn = layers.Conv2D(filters=self.num_filters, 
								 kernel_size = self.kernel, 
								 kernel_initializer=self.kernel_initializer, 
								 strides=self.strides, 
								 padding=self.padding, 
		...
		self.minpool = min_pool2D(pool_size=self.kernel, strides=self.strides, padding=self.padding)
		self.maxpool = layers.MaxPooling2D(pool_size=self.kernel, strides=self.strides, padding=self.padding)
		self.avgpool = layers.AveragePooling2D(pool_size=self.kernel, strides=self.strides, padding=self.padding)
...
class bridge_residual_conv2D_block(layers.Layer):                  # this block is to be used while changing the number of channels.
	...
	def build(self, input_shape):
		...
		self.conv_skip = layers.Conv2D(filters=self.num_filters, 
								 kernel_size = 1, 
								 kernel_initializer=self.kernel_initializer)     # no regularization on this one
...
class residual_conv2Dtrans_block(layers.Layer):
	...
	def build(self, input_shape):
		...
		self.convtrans_bn = conv2dtrans_block(self.num_filters, 
										kernel=self.kernel, 
										kernel_initializer=self.kernel_initializer, 
										padding = self.padding, 
										use_bn=self.use_bn, 
		...
		self.convtrans_wobn = layers.Conv2DTranspose(filters=self.num_filters, 
											   kernel_size=self.kernel, 
											   kernel_initializer=self.kernel_initializer, 
											   padding=self.padding, 
											   kernel_regularizer=self.kernel_regularizer)
		...
		self.zeropad_asym = layers.ZeroPadding2D(padding=((p,self.kernel-1-p),(p,self.kernel-1-p)))
		self.doublezeropad = layers.ZeroPadding2D(padding= self.kernel-1)
...
class bridge_residual_conv2Dtrans_block(layers.Layer):			# this block is to be used while changing the number of channels.
	...
	def build(self, input_shape):
		...
		self.convtrans_skip = layers.Conv2DTranspose(filters=self.num_filters, 
											   kernel_size = 1, 
											   kernel_initializer=self.kernel_initializer)     # no regularization on this one
